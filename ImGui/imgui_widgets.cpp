...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 text_act = ImColor(vars.misc.menu_color.r(), vars.misc.menu_color.g(), vars.misc.menu_color.b(), 255);
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 icon_act = ImColor(vars.misc.menu_color.r(), vars.misc.menu_color.g(), vars.misc.menu_color.b(), 255);
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, ImColor(51, 51, 51), true, 2.f * c_menu::get().dpi_scale);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(48 / 255.f, (119) / 255.f, (233) / 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    const ImVec4 text_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    const ImVec4 icon_act = ImVec4(48 / 255.f, (119) / 255.f, (233) / 255.f, 1.f);

    RenderFrame(bb.Min, bb.Max, ImColor(51, 51, 51), true, 2.f * c_menu::get().dpi_scale);

    window->DrawList->PushClipRect(bb.Min, bb.Max, false);
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(48 / 255.f, (119) / 255.f, (233) / 255.f, it_circle->second.alpha)), 60);
...
bool ImGui::PlusButton(const char* label, bool type, const ImVec2& size_arg, const char* uniq_id, ImColor col, bool config)
{
    ...
    if (config)
        RenderFrame(bb.Min, bb.Max, ImColor(51 / 255.f, 51 / 255.f, 51 / 255.f, 1.f - it_hover->second), true, 24 - (20 * it_hover->second));
    ...
    csgo.gui.open_pop = it_open->second.play;
    csgo.gui.pop_anim = it_open->second.mult;
    if (!type)
    {
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImColor(vars.misc.menu_color.r(), vars.misc.menu_color.g(), vars.misc.menu_color.b(), 255);
...
bool ImGui::tab(const char* icon, const char* label, const char* desc, bool selected, int ssize)
{
    ...
    it_filled->second = ImClamp(it_filled->second + (2.15f * ImGui::GetIO().DeltaTime * (selected ? 1.f : -1.2f)), 0.f, 1.f);
    ...
    auto main_text = main_text_anim.find(id);
    if (main_text == main_text_anim.end())
    {
        main_text_anim.insert({ id, 0.f });
        main_text = main_text_anim.find(id);
    }
    main_text->second = ImClamp(main_text->second + (2.15f * ImGui::GetIO().DeltaTime * (ssize > 220 ? 1.f : -2.8f)), 0.f, 1.f);
    main_text->second *= min(GetStyle().Alpha * 1.5, 1.f);

    static std::map<ImGuiID, float> main_text_anim_act;
    auto act_m = main_text_anim_act.find(id);
    if (main_text == main_text_anim_act.end())
    {
    ...
    //37, 37, 40
    ImVec4 col = ImLerp(ImVec4{ 77 / 255.f, 77 / 255.f, 80 / 255.f, 0.9f }, ImVec4{ vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, 1.f }, it_hover->second);
    ImVec4 col2 = ImLerp(ImVec4{ 122 / 255.f, 122 / 255.f, 122 / 255.f, main_text->second }, ImVec4{ vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, main_text->second }, ImClamp(it_filled->second, 0.f, 1.f));
    ImVec4 col3 = ImLerp(ImVec4{ 32 / 255.f, 32 / 255.f, 35 / 255.f, 0.4f }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, 1.f }, t);

    if (ssize > 220)
        ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + ssize - 3, bb.Min.y), ImVec2(bb.Min.x + ssize, bb.Max.y), ImColor(vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, it_lined->second), 5, ImDrawCornerFlags_Left);
    ...
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (42 / 2 - label_size.x / 2), bb.Min.y + (50 / 2 - label_size.y / 2)), ImColor(vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, it_filled->second), icon);
    ...
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 45, bb.Max.y - CalcTextSize(desc).y * 2), ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, main_text->second), desc);
    ImGui::PopFont();
...
bool ImGui::subtab(const char* label, bool selected, float sizex, int cout, bool reverse)
{
    ...
    //37, 37, 40
    ImVec4 col = ImLerp(ImVec4{ 94 / 255.f, 94 / 255.f, 94 / 255.f, 0.7f * al }, ImVec4{ vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, 1.f * al }, t * al);
    ...
    if (it_filled->second > 0)
        if (!reverse)
            ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + size.x * min(t * 2, 1), bb.Min.y + 2 * min(t, 1)), ImColor(vars.misc.menu_color.r(), vars.misc.menu_color.g(), vars.misc.menu_color.b(), (int)((255 * t) * al)), 12, ImDrawCornerFlags_Bot);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto al = float(ImGui::GetStyle().Alpha);
    RenderFrame(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(51, 51, 51, int(255 * al)));

    window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(pad - 1, pad - 1), check_bb.Max - ImVec2(pad - 1, pad - 1), ImColor(8 / 255.f, (vars.misc.menu_color.g() - 40) / 255.f, (vars.misc.menu_color.g() - 40) / 255.f, it_filled->second), ImColor(8 / 255.f, (vars.misc.menu_color.g() - 60) / 255.f, (vars.misc.menu_color.b() - 60) / 255.f, it_filled->second), ImColor(8 / 255.f, (vars.misc.menu_color.g() - 40) / 255.f, (vars.misc.menu_color.b() - 40) / 255.f, it_filled->second), ImColor(8 / 255.f, (vars.misc.menu_color.g() - 80) / 255.f, (vars.misc.menu_color.b() - 80) / 255.f, it_filled->second));
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, it_filled->second));
    if (label_size.x > 0.0f) {
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float expected_w = window->Size.x - 16;
    ...
    const ImVec4 hover_act = ImColor(vars.misc.menu_color.r(), vars.misc.menu_color.g(), vars.misc.menu_color.b(), 255);
    ...
    const ImVec4 arrow_act = ImColor(vars.misc.menu_color.r(), vars.misc.menu_color.g(), vars.misc.menu_color.b(), 255);
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(55, 55, 55), 2);
    ...
    if (vars.scripts.developer_mode && ImGui::IsItemHovered())
    {
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(expected_w, 0.0f), ImVec2(expected_w, ((CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)) * it_open->second)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9), ImVec2(frame_bb.Max.x - 2, frame_bb.Max.y - 3 - 8), ImColor(51, 51, 51), 4);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9), ImVec2(grab_bb.Max.x - 2, grab_bb.Max.y - 9), ImColor(vars.misc.menu_color.r() / 255.f, (vars.misc.menu_color.g()) / 255.f, (vars.misc.menu_color.b()) / 255.f, 0.9f * GetStyle().Alpha), 4);
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - 2, grab_bb.Min.y + 6), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 8), ImColor(vars.misc.menu_color.r() / 255.f, (vars.misc.menu_color.g()) / 255.f, (vars.misc.menu_color.b()) / 255.f, 1 * GetStyle().Alpha), 3);
    }
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    ...
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 2 - CalcTextSize(value_buf).x, window->DC.CursorPos.y - 30), GetColorU32(ImGuiCol_Text), value_buf, value_buf_end);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        csgo.globals.focused_on_input = true;
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(51,51,51), true, 2);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size, const bool color)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
