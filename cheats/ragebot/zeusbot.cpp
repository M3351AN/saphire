...
void zeusbot::run(CUserCmd* cmd)
{
	...
	if (!vars.ragebot.zeus_bot)
		return;

	if (csgo.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER)
		return;
...
void zeusbot::scan(adjust_data* record, scan_data& data)
{
	...
	for (hitbox = HITBOX_PELVIS; hitbox <= HITBOX_UPPER_CHEST; ++hitbox)
	{
		...
		if (!record->bot)
		{
			...
			if (record->matrixes_data.zero[0].GetOrigin() == record->matrixes_data.first[0].GetOrigin() || record->matrixes_data.zero[0].GetOrigin() == record->matrixes_data.second[0].GetOrigin() || record->matrixes_data.first[0].GetOrigin() == record->matrixes_data.second[0].GetOrigin())
				safe = 0.0f;
			else if (!hitbox_intersection(record->player, record->matrixes_data.zero, hitbox, csgo.globals.eye_pos, point.point, &safe))
				safe = 0.0f;
			else if (!hitbox_intersection(record->player, record->matrixes_data.first, hitbox, csgo.globals.eye_pos, point.point, &safe))
				safe = 0.0f;
			else if (!hitbox_intersection(record->player, record->matrixes_data.second, hitbox, csgo.globals.eye_pos, point.point, &safe))
				safe = 0.0f;
	...
	for (auto& point : points)
	{
		...
		auto fire_data = autowall::get().wall_penetration(csgo.globals.eye_pos, point.point, record->player);
...
void zeusbot::fire(CUserCmd* cmd)
{
	...
	if (vars.misc.events_to_log[EVENTLOG_HIT])
