...
void knifebot::scan_targets()
{
	...
	for (auto& target : aim::get().targets)
	{
		if (target.history_record->valid())
		{
			...
			{
				auto last_distance = csgo.globals.eye_pos.DistTo(target.last_record->origin);
				auto history_distance = csgo.globals.eye_pos.DistTo(target.history_record->origin);
...
int GetMinimalHp() {
	if (TICKS_TO_TIME(csgo.globals.fixed_tickbase) > (csgo.globals.weapon->m_flNextPrimaryAttack() + 0.4f))
		return 34;
...
void knifebot::fire(CUserCmd* cmd)
{
	...
	auto vecDelta = vecMins - csgo.globals.eye_pos;

	if (vecDelta.Length() > 60.0f)
		return;

	vecDelta.Normalize();
	auto delta = fabs(math::normalize_yaw(final_target.record->angles.y - math::calculate_angle(final_target.record->player->get_shoot_position(), csgo.local()->GetAbsOrigin()).y));

	if (final_target.record->player->m_iHealth() > 63 && delta < 120.0f || final_target.record->player->m_iHealth() < 37)
	{
...
int knifebot::determinate_hit_type(bool stab_type, const Vector& delta)
{
	...
	auto end = csgo.globals.eye_pos + delta * minimum_distance;
	...
	ray.Init(csgo.globals.eye_pos, end, Vector(-16.0f, -16.0f, -18.0f), Vector(16.0f, 16.0f, 18.0f));
	...
	auto final_delta = final_target.record->origin - csgo.globals.eye_pos;
