...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	if (!player_info.fakeplayer && csgo.local()->is_alive() && e->m_iTeamNum() != csgo.local()->m_iTeamNum() && ticks_chocked >= 1)
	{
		...
		case RESOLVER_ZERO:
<<<<<<< Updated upstream
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + floor(e->get_max_desync_delta() * 0.25f));
			break;
		case RESOLVER_FIRST:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + floor(e->get_max_desync_delta()*0.7f));
=======
