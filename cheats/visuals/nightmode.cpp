...
void nightmode::modulate(MaterialHandle_t i, IMaterial* material, bool backup = false) 
{
	...
	if (strstr(name, crypt_str("World")))
	{
		...
		material->ColorModulate((float)vars.esp.world_color.r() / 255.0f, (float)vars.esp.world_color.g() / 255.0f, (float)vars.esp.world_color.b() / 255.0f);
	}
	else if (strstr(name, crypt_str("StaticProp")))
	{
		...
		material->AlphaModulate((float)vars.esp.world_color.a() / 255.0f);
		material->ColorModulate((float)(vars.esp.world_color.r() + 20) / 255.0f, (float)(vars.esp.world_color.g() + 20) / 255.0f, (float)(vars.esp.world_color.b() + 20) / 255.0f);
	}
}
