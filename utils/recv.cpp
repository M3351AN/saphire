...
static void viewModelSequence(recvProxyData& data, void* arg2, void* arg3) noexcept
{
	if (csgo.local())
	{
		if (const auto activeWeapon = csgo.local()->m_hActiveWeapon())
		{
			...
			else if (vars.skins.rare_animations && activeWeapon->m_iItemDefinitionIndex() == WEAPON_DEAGLE && data.value._int == 7)
				data.value._int = 8;
			else if (vars.skins.rare_animations && activeWeapon->m_iItemDefinitionIndex() == WEAPON_KNIFE_FALCHION && data.value._int == 12)
				data.value._int = 13;
		}
	}
...
{
	data.value._int = vars.ragebot.enable;
